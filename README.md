...
5. **远程 Web-SSH 管理**
   - 远程本地登录会代理显示和管理目标机的系统信息与 Docker，而不会误用中转机数据
   - 登录时可选择远端实际可用的 Shell；`bash` 与 `sh` 可作为没有 `zsh` 时的回退
   - 直接 SSH 连接不显示系统信息和 Docker 管理，避免将本机状态误标为远端状态
...
- 支持 Chrome 75：页面使用本地预编译 CSS 与兼容 JavaScript，不依赖浏览器端 Tailwind 编译或外部图标服务。
